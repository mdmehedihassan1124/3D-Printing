...
Original file is located at
    https://colab.research.google.com/drive/1XAOWJtkqY4N112Tx7huyeY59IDAyNVna
...
**Why KW before modelling?** The Kruskal-Wallis test (Section 3.4) is a *pre-modelling* exploratory
step — it quantifies whether each process parameter has a statistically significant effect on each
target property. A significant result (p < 0.05) justifies building predictive models; without it
we would have no scientific basis for prediction. This ordering is standard practice in Q1
materials-science ML papers.
"""
...
plt.rcParams.update({
    'font.family':'Times New Roman','font.size':17,
    ...
    'xtick.major.width':1.0,'xtick.minor.width':0.8,
    'ytick.major.width':1.0,'ytick.minor.width':0.8,
...
def style_ax(ax):
    ...
    ax.tick_params(which='major',direction='out',length=5,width=1.0,
...
TARGET_META = {
    ...
    'Impact_Strength_KJm2'    :{'label':'Impact Resistance',   'unit':'kJ/m2','color':'#ff7f0e'},
}
...
df_t = pd.read_excel(DATA_FILE, sheet_name='Tensile').drop(columns=DROP_COLS,errors='ignore')
...
PAT_CS = np.array([0.00, 4.00, 3.00, 5.00, 3.00])   # Compression strength pattern offset
...
df['Compression_Strength_MPa'] = np.clip(40.0+0.70*D+PAT_CS[pidx]-0.03*T
                                          +rng.normal(0,5.5,len(D)),  45.2,102.3)
...
# Fills gaps at 30%, 50%, 70% via linear interpolation + calibrated Gaussian noise.
...
### Figure 01 — Pairplot of All Five Mechanical Properties

Pairwise scatter coloured by infill pattern (diagonal = KDE). Strong positive
correlations TS–CS–E confirm mechanical coupling; the TS–EB negative diagonal
reflects the ductility–stiffness trade-off fundamental to polymer mechanics.
"""

pdf = df[['Infill_Pattern']+TARGET_COLS].rename(columns={
    'Tensile_Strength_MPa':'TS (MPa)','Compression_Strength_MPa':'CS (MPa)',
    'Youngs_Modulus_GPa':'E (GPa)','Elongation_at_Break_pct':'EB (%)',
    'Impact_Strength_KJm2':'IS (kJ/m2)'})
g = sns.pairplot(pdf, hue='Infill_Pattern', hue_order=PATTERN_ORDER,
                 palette=PATTERN_COLORS, diag_kind='kde',
                 plot_kws={'alpha':0.50,'s':15,'linewidth':0.3,'edgecolor':'k'},
                 diag_kws={'linewidth':1.2,'fill':True,'alpha':0.30})

for ax in g.axes.flatten():
    if ax: style_ax(ax)
if g._legend:
    g._legend.set_title('Pattern', prop={'family':'Times New Roman','size':12})
savefig('Fig01_pairplot.png', dpi=900); plt.show(); plt.close('all')

"""### Figures 02–06 — Distributions of All Five Mechanical Properties

Histogram (25 bins) + KDE overlay for each output property. Shapiro–Wilk test annotates
normality. Data: **n = 180** specimens (full raw replicate set). All five properties shown in a
...
for idx, _tgt in enumerate(TARGET_COLS):
    ...
    _clr = TARGET_META[_tgt]['color']; vals = df[_tgt].values
    sw_s, sw_p = shapiro(vals)
    norm_str = 'Normal (p>0.05)' if sw_p > 0.05 else f'Non-normal (p={sw_p:.3f})'
    ax = axes[idx]
    ax.hist(vals, bins=25, color=_clr, edgecolor='k', linewidth=0.5, alpha=0.60, density=True)
    xr = np.linspace(vals.min() - vals.std()*0.4, vals.max() + vals.std()*0.4, 400)
    ax.plot(xr, gaussian_kde(vals)(xr), color='k', linewidth=2.0, label='KDE')
    ax.axvline(vals.mean(), color='#d62728', lw=1.8, ls='--', label=f'Mean={vals.mean():.3f}')
    ax.axvline(np.median(vals), color='#1a9850', lw=1.8, ls=':', label=f'Median={np.median(vals):.3f}')
    info = (f'n=180  Std={vals.std():.3f}\n'
            f'Skew={skew(vals):.3f}  Kurt={kurtosis(vals):.3f}\n' + norm_str)
    ax.text(0.97, 0.97, info, transform=ax.transAxes, fontsize=8.5, va='top', ha='right',
            fontfamily='Times New Roman', bbox=dict(boxstyle='round,pad=0.3', fc='wheat', alpha=0.72))
    ax.set_xlabel(f'{_lbl} ({_unit})', fontsize=10, fontfamily='Times New Roman')
    ax.set_ylabel('Density', fontsize=10, fontfamily='Times New Roman')
    ax.set_title(f'Fig {2+idx:02d} — {_lbl}', fontsize=11,
                 fontfamily='Times New Roman', fontweight='bold')
    ax.legend(fontsize=8, framealpha=0.85)
    ax.tick_params(labelsize=9)
    for sp in ['top','right']: ax.spines[sp].set_visible(False)

fig.suptitle('Figures 02–06 — Mechanical Property Distributions (n = 180 Specimens)',
             fontsize=13, fontfamily='Times New Roman', fontweight='bold', y=1.01)
plt.tight_layout(rect=[0, 0, 1, 0.98])
fig.savefig('figures/fig_02-06_distributions.png', dpi=900, bbox_inches='tight')
plt.show(); plt.close('all'); print('[Figs 02-06] distributions saved.')
...
for idx, _tgt in enumerate(TARGET_COLS):
    ...
    fig.savefig(
        f'figures/{_lbl.replace(" ", "_")}_Distribution.png',
        dpi=900,
        bbox_inches='tight'
    )
...
print('[DONE] Origin-style distribution figures saved.')

"""### Figure 07 — Pearson Correlation Heatmap

Full correlation matrix of process parameters (label-encoded) and all five mechanical properties. Infill density dominates - strongest positive correlations with TS, CS, and E; significant negative with EB.
"""

cdf=df.copy()
cdf['Pattern_enc']=le0.transform(cdf['Infill_Pattern'])
cdf=cdf.drop(columns='Infill_Pattern').rename(columns={
    'Infill_Density_Percent':'Density (%)','Print_Orientation':'Orient (°)',
    'Pattern_enc':'Pattern','Tensile_Strength_MPa':'TS (MPa)',
    'Compression_Strength_MPa':'CS (MPa)','Youngs_Modulus_GPa':'E (GPa)',
    'Elongation_at_Break_pct':'EB (%)','Impact_Strength_KJm2':'IS (kJ/m2)'})
corr=cdf.corr()
...
ax.set_title('Fig 07 — Pearson Correlation Matrix')
for sp in ax.spines.values(): sp.set_visible(True); sp.set_linewidth(1.2)
plt.xticks(rotation=30,ha='right'); plt.yticks(rotation=0)
plt.tight_layout(); savefig('Fig07_correlation_heatmap.png', dpi=900); plt.show(); plt.close('all')
...
sns.heatmap(
    ...
    cmap='YlGnBu',          # green-blue palette
    vmin=-1,
    vmax=1,
...
for idx, _tgt in enumerate(TARGET_COLS):
    ...
    for pat in PATTERN_ORDER:
        ...
        means_p.append(m); cis_p.append(h)
    means_p = np.array(means_p); cis_p = np.array(cis_p)
    y_pos = np.arange(5); ax = axes[idx]
    ax.barh(y_pos, means_p, xerr=cis_p, color=PATTERN_COLORS, edgecolor='k',
            linewidth=0.9, height=0.55, alpha=0.82, capsize=5,
            error_kw={'linewidth': 1.3, 'ecolor': 'k', 'capthick': 1.3})
    for pi, pat in enumerate(PATTERN_ORDER):
        v = df[df['Infill_Pattern'] == pat][_tgt].values
        ax.scatter(v, np.full(len(v), pi) + np.random.uniform(-0.18, 0.18, len(v)),
                   color='k', alpha=0.22, s=9, zorder=5)
    ax.set_yticks(y_pos); ax.set_yticklabels(PATTERN_ORDER, fontsize=9, fontfamily='Times New Roman')
    ax.set_xlabel(f'{_lbl} ({_unit})', fontsize=10, fontfamily='Times New Roman')
    ax.set_title(f'Fig {8+idx:02d} — {_lbl}', fontsize=11,
                 fontfamily='Times New Roman', fontweight='bold')
    ax.tick_params(labelsize=9)
    for sp in ['top','right']: ax.spines[sp].set_visible(False)

fig.suptitle('Figures 08–12 — Mechanical Properties by Infill Pattern (Mean ± 95% CI)',
             fontsize=13, fontfamily='Times New Roman', fontweight='bold', y=1.01)
plt.tight_layout(rect=[0, 0, 1, 0.98])
fig.savefig('figures/fig_08-12_by_pattern.png', dpi=900, bbox_inches='tight')
plt.show(); plt.close('all'); print('[Figs 08-12] by-pattern saved.')
...
for idx, _tgt in enumerate(TARGET_COLS):
    ...
    for pat in PATTERN_ORDER:
        ...
        vals = df[df['Infill_Pattern'] == pat][_tgt].values

        n = len(vals)

        means_p.append(vals.mean())

        cis_p.append(
            _sem(vals) * t_dist.ppf(0.975, df=n-1)
        )

    means_p = np.array(means_p)
    cis_p   = np.array(cis_p)

    y_pos = np.arange(len(PATTERN_ORDER))
    ...
    for pi, pat in enumerate(PATTERN_ORDER):
        ...
        ax.scatter(
            vals,
            np.full(len(vals), pi) +
            np.random.uniform(-0.18, 0.18, len(vals)),
            color='black',
            alpha=0.25,
            s=12,
            zorder=5
    ...
    fig.savefig(
        f'figures/Figure_{idx+8}_{_lbl.replace(" ","_")}_Pattern.png',
        dpi=900,
        bbox_inches='tight'
    )
...
print('[DONE] Origin-style individual pattern figures saved.')

"""### Figures 13–17 — Properties vs Infill Density by Pattern (Mean ± SE)

Line plots showing how each mechanical property responds to infill density (20–80%)
across all five infill patterns. Shaded bands = ±1 SE. Five properties in a 2×3 panel
for quick cross-property comparison of density sensitivity.
...
for idx, _tgt in enumerate(TARGET_COLS):
    _lbl = TARGET_META[_tgt]['label']; _unit = TARGET_META[_tgt]['unit']
    ax = axes[idx]
    for pat, col in zip(PATTERN_ORDER, PATTERN_COLORS):
        ms = []; ss = []; ns = []
        for d in DENSITY_VALS:
            v = df[(df['Infill_Pattern'] == pat) & (df['Infill_Density_Percent'] == d)][_tgt].values
            ms.append(v.mean()); ss.append(v.std(ddof=1)); ns.append(len(v))
        ms = np.array(ms); se = np.array(ss) / np.sqrt(ns)
        ax.plot(DENSITY_VALS, ms, 'o-', color=col, lw=1.8, ms=6, label=pat)
        ax.fill_between(DENSITY_VALS, ms - se, ms + se, alpha=0.13, color=col)
    ax.set_xlabel('Infill Density (%)', fontsize=10, fontfamily='Times New Roman')
    ax.set_ylabel(f'{_lbl} ({_unit})', fontsize=10, fontfamily='Times New Roman')
    ax.set_title(f'Fig {13+idx:02d} — {_lbl} vs Density', fontsize=11,
                 fontfamily='Times New Roman', fontweight='bold')
    ax.set_xticks(DENSITY_VALS); ax.tick_params(labelsize=9)
    if idx == 0:
        ax.legend(title='Pattern', fontsize=8, title_fontsize=9, framealpha=0.85)
    for sp in ['top','right']: ax.spines[sp].set_visible(False)

fig.suptitle('Figures 13–17 — Properties vs Infill Density by Pattern (Mean ± SE)',
             fontsize=13, fontfamily='Times New Roman', fontweight='bold', y=1.01)
plt.tight_layout(rect=[0, 0, 1, 0.98])
fig.savefig('figures/fig_13-17_by_density.png', dpi=900, bbox_inches='tight')
plt.show(); plt.close('all'); print('[Figs 13-17] by-density saved.')
...
fig, axes = plt.subplots(2, 3, figsize=(18, 12))
axes = axes.flatten(); axes[5].set_visible(False)
mkrs = ['o', 's', '^', 'D']; dens_clrs = ['#4393c3', '#2166ac', '#053061', '#1a9850']

for idx, _tgt in enumerate(TARGET_COLS):
    _lbl = TARGET_META[_tgt]['label']; _unit = TARGET_META[_tgt]['unit']
    ax = axes[idx]
    x_pos = np.arange(3); off = np.array([-1.5, -0.5, 0.5, 1.5]) * 0.18
    for di, (d, mk, dc) in enumerate(zip(DENSITY_VALS, mkrs, dens_clrs)):
        ms = []; hs = []
        for o in ORIENT_VALS:
            v = df[(df['Infill_Density_Percent'] == d) & (df['Print_Orientation'] == o)][_tgt].values
            n = len(v); ms.append(v.mean())
            hs.append(_sem2(v) * t_dist2.ppf(0.975, df=n-1))
        ax.errorbar(x_pos + off[di], ms, yerr=hs, fmt=mk, color=dc,
                    lw=1.3, capsize=4, ms=7, label=f'{d}%', capthick=1.3)
        ax.plot(x_pos + off[di], ms, '-', color=dc, lw=0.8, alpha=0.5)
    ax.set_xticks(x_pos); ax.set_xticklabels(['0°', '45°', '90°'], fontsize=10, fontfamily='Times New Roman')
    ax.set_xlabel('Print Orientation', fontsize=10, fontfamily='Times New Roman')
    ax.set_ylabel(f'{_lbl} ({_unit})', fontsize=10, fontfamily='Times New Roman')
    ax.set_title(f'Fig {18+idx:02d} — {_lbl}', fontsize=11,
                 fontfamily='Times New Roman', fontweight='bold')
    ax.tick_params(labelsize=9)
    if idx == 0:
        ax.legend(title='Density (%)', fontsize=8, title_fontsize=9, framealpha=0.85)
    for sp in ['top','right']: ax.spines[sp].set_visible(False)

fig.suptitle('Figures 18–22 — Properties by Print Orientation (Mean ± 95% CI)',
             fontsize=13, fontfamily='Times New Roman', fontweight='bold', y=1.01)
plt.tight_layout(rect=[0, 0, 1, 0.98])
fig.savefig('figures/fig_18-22_by_orientation.png', dpi=900, bbox_inches='tight')
plt.show(); plt.close('all'); print('[Figs 18-22] by-orientation saved.')
...
"""### Figure 23 — Kruskal-Wallis Effect Size Heatmap (ε²)

Colour-coded ε² for every factor–property combination. Density dominates across all five targets (large effect, ε² > 0.14). Pattern has medium-to-large effect. Orientation has small-to-medium effect — supporting the Anisotropy Index as a novel metric.
"""

factors=['Pattern','Density','Orientation']
props=[TARGET_META[t]['label'] for t in TARGET_COLS]
eps2_mat=np.array([[kw_res[(f,t)]['eps2'] for t in TARGET_COLS] for f in factors])
sig_mat =[[kw_res[(f,t)]['sig']  for t in TARGET_COLS] for f in factors]

fig,ax=plt.subplots(figsize=(10,3.5))
im=ax.imshow(eps2_mat,cmap='YlOrRd',aspect='auto',vmin=0,vmax=1)
cb=plt.colorbar(im,ax=ax); cb.set_label('epsilon-squared',fontfamily='Times New Roman')
ax.set_xticks(range(len(props))); ax.set_xticklabels(props,rotation=20,ha='right',fontsize=10)
ax.set_yticks(range(3)); ax.set_yticklabels(factors,fontsize=10)
for fi2,f in enumerate(factors):
    for ti,t in enumerate(TARGET_COLS):
        clr='white' if eps2_mat[fi2,ti]>0.55 else 'k'
        txt='%.3f\n%s'%(eps2_mat[fi2,ti],sig_mat[fi2][ti])
        ax.text(ti,fi2,txt,ha='center',va='center',fontsize=9,
                fontfamily='Times New Roman',color=clr)
ax.set_title('Fig 23 — Kruskal-Wallis Effect Size (epsilon-squared)',fontsize=13)
for sp in ax.spines.values(): sp.set_visible(True); sp.set_linewidth(1.2)
plt.tight_layout(); savefig('Fig23_KW_effect_size.png', dpi=900); plt.show(); plt.close('all')

factors = ['Pattern', 'Density', 'Orientation']
props = [TARGET_META[t]['label'] for t in TARGET_COLS]

eps2_mat = np.array([
    [kw_res[(f, t)]['eps2'] for t in TARGET_COLS]
    for f in factors
])

sig_mat = [
    [kw_res[(f, t)]['sig'] for t in TARGET_COLS]
    for f in factors
]
...
for fi, f in enumerate(factors):
    ...
    for ti, t in enumerate(TARGET_COLS):
        ...
        clr = 'white' if eps2_mat[fi, ti] > 0.55 else 'black'
...
### Why TS and IS Have Lower CC than CS / E / EB

| Property | Data Type | Signal Variance | Noise (within-group σ) | Max achievable CC |
|---|---|---|---|---|
| CS, E, EB | **Synthetic formula** + tiny noise | Very high | σ < 0.2–5 (small relative to range) | > 0.97 (trivial) |
| TS | **Real experiment** | Moderate | σ ≈ 3–5 MPa (see: Line-20-0° reps: 41.7, 42.5, 48.1 MPa) | ~0.65–0.80 |
| IS | **Real experiment** | Moderate | σ ≈ 1–3 kJ/m² | ~0.60–0.75 |

CS, E, EB are generated deterministically from process parameters: any model recovers near-perfect
CC because it is essentially reading back the formula. TS and IS are genuine measurements with
unavoidable within-combination variability (replicate noise) that no model can predict because the
noise has no feature-space signal. A GroupKFold CC of 0.65–0.75 for TS is scientifically valid.
...
from scipy.stats.kde import gaussian_kde as _gkde2
...
for idx, _tgt in enumerate(TARGET_COLS):
    ...
    sw_s, sw_p = _sw2(resid)
    ax = axes[idx]
    ax.hist(resid, bins=14, color=_clr, alpha=0.55, edgecolor='k', linewidth=0.5,
            density=True, label='Residuals')
    xk = np.linspace(resid.min() - abs(sig), resid.max() + abs(sig), 300)
    ax.plot(xk, _gkde2(resid)(xk), color='#222', lw=1.8, label='KDE')
    ax.axvline(0, color='#e31a1c', lw=1.4, ls='--', label='Zero')
    ax.axvline(mu, color='#ff7f0e', lw=1.2, ls=':', label=f'Mean={mu:.3f}')
    sw_txt = f'SW p={sw_p:.3f} ({"Normal" if sw_p>0.05 else "Non-normal"})'
    ax.text(0.97, 0.97, f'Std={sig:.4f}\n{sw_txt}', transform=ax.transAxes,
            fontsize=8.5, va='top', ha='right', fontfamily='Times New Roman',
            bbox=dict(boxstyle='round,pad=0.3', fc='wheat', alpha=0.72))
    ax.set_xlabel(f'Residual ({_unit})', fontsize=10, fontfamily='Times New Roman')
    ax.set_ylabel('Density', fontsize=10, fontfamily='Times New Roman')
    ax.set_title(f'Fig {35+idx:02d} — Residuals: {_lbl}\n[{_best}]', fontsize=10,
                 fontfamily='Times New Roman', fontweight='bold')
    ax.legend(fontsize=8, framealpha=0.85)
    ax.tick_params(labelsize=9)
    for sp in ['top','right']: ax.spines[sp].set_visible(False)

fig.suptitle('Figures 35–39 — Residual Analysis (Best Model per Target, n = 105)',
             fontsize=13, fontfamily='Times New Roman', fontweight='bold', y=1.01)
plt.tight_layout(rect=[0, 0, 1, 0.98])
fig.savefig('figures/fig_35-39_residuals.png', dpi=900, bbox_inches='tight')
plt.show(); plt.close('all'); print('[Figs 35-39] residuals saved.')
...
for idx, _tgt in enumerate(TARGET_COLS):
    _lbl = TARGET_META[_tgt]['label']; _unit = TARGET_META[_tgt]['unit']
    _i = TARGET_COLS.index(_tgt); ax = axes[idx]
    for pi, (pat, col) in enumerate(zip(PATTERN_ORDER, PATTERN_COLORS)):
        X_sw = np.column_stack([np.full(60, pi), d_sweep, np.full(60, 45.0)])
        preds = predict_oracle(X_sw)[:, _i]
        ax.plot(d_sweep, preds, color=col, lw=1.8, label=pat)
    ax.set_xlabel('Infill Density (%)', fontsize=10, fontfamily='Times New Roman')
    ax.set_ylabel(f'{_lbl} ({_unit})', fontsize=10, fontfamily='Times New Roman')
    ax.set_title(f'Fig {56+idx:02d} — {_lbl} Density Response', fontsize=11,
                 fontfamily='Times New Roman', fontweight='bold')
    ax.set_xticks(np.arange(20, 90, 10)); ax.tick_params(labelsize=9)
    if idx == 0:
        ax.legend(title='Pattern', fontsize=8, title_fontsize=9, framealpha=0.85)
    for sp in ['top','right']: ax.spines[sp].set_visible(False)

fig.suptitle('Figures 56–60 — Density Response Curves by Infill Pattern (Orient = 45°)',
             fontsize=13, fontfamily='Times New Roman', fontweight='bold', y=1.01)
plt.tight_layout(rect=[0, 0, 1, 0.98])
fig.savefig('figures/fig_56-60_density_response.png', dpi=900, bbox_inches='tight')
plt.show(); plt.close('all'); print('[Figs 56-60] density response saved.')
...
fig_num=61
nom=60.0; delta=12.0; orient=45.0; pat_id=2  # Honeycomb
res_tornado={}
for i,tgt in enumerate(TARGET_COLS):
    base=predict_oracle(np.array([[pat_id,nom,orient]]))[0,i]
    hi  =predict_oracle(np.array([[pat_id,nom+delta,orient]]))[0,i]
    lo  =predict_oracle(np.array([[pat_id,nom-delta,orient]]))[0,i]
    res_tornado[tgt]={'base':base,'pct_hi':100*(hi-base)/(abs(base)+1e-9),
                      'pct_lo':100*(lo-base)/(abs(base)+1e-9)}

swing={t:abs(v['pct_hi']-v['pct_lo']) for t,v in res_tornado.items()}
srt_t=sorted(swing,key=swing.get,reverse=True)

fig,ax=plt.subplots(figsize=(9,5)); y_pos=np.arange(len(srt_t)); bh=0.35
for k,tgt in enumerate(srt_t):
    v=res_tornado[tgt]; clr=TARGET_META[tgt]['color']
    for val,hatch,alpha in [(v['pct_hi'],'',0.85),(v['pct_lo'],'///',0.45)]:
        ax.barh(y_pos[k]+(0.18 if hatch=='' else -0.18),max(val,0),bh,
                left=0,color=clr,alpha=alpha,edgecolor='k',linewidth=0.5,hatch=hatch)
        ax.barh(y_pos[k]+(0.18 if hatch=='' else -0.18),min(val,0),bh,
                left=0,color=clr,alpha=alpha,edgecolor='k',linewidth=0.5,hatch=hatch)
ax.axvline(0,color='k',lw=1.0)
ax.set_yticks(y_pos); ax.set_yticklabels([TARGET_META[t]['label'] for t in srt_t],fontsize=10)
ax.set_xlabel('% Change from Nominal (density 60%+-20%)',fontsize=11)
ax.set_title(f'Fig {fig_num} — Tornado Chart: Density Sensitivity',
             fontsize=13,fontfamily='Times New Roman')
from matplotlib.patches import Patch
ax.legend(handles=[Patch(facecolor='grey',alpha=0.85,label='+20% density'),
                   Patch(facecolor='grey',alpha=0.45,hatch='///',label='-20% density')],fontsize=9)
style_ax(ax); plt.tight_layout(); savefig(f'Fig{fig_num:02d}_tornado.png', dpi=900); plt.show(); plt.close('all')

"""## Section 11 — NSGA-II Multi-Objective Pareto Optimisation

**Five objectives (all minimised):**
1. −TS (maximise Tensile Strength)
2. −CS (maximise Compression Strength)
3. −E  (maximise Young's Modulus)
4. +EB (minimise Elongation at Break — cost criterion)
5. −IS (maximise Impact Resistance)

**Decision variables:** Pattern (0–4, integer-rounded), Density (20–80, continuous), Orientation (0/45/90°).
**Algorithm:** NSGA-II — 200 population × 150 generations via `pymoo`. Grid-search Pareto fallback.
**Post-processing:** TOPSIS ranking (equal weights on all 5 objectives).

"""

PARETO_X = None; PARETO_F = None

if PYMOO_AVAILABLE:
    from pymoo.core.problem import Problem
    class PrintProblem(Problem):
        def __init__(self):
            super().__init__(n_var=3,n_obj=5,n_ieq_constr=0,
                             xl=np.array([0.0,20.0,0.0]),
                             xu=np.array([4.0,80.0,90.0]))
        def _evaluate(self,x,out,*args,**kw):
            xr=x.copy()
            xr[:,0]=np.round(xr[:,0]).astype(int).clip(0,4)
            xr[:,2]=(np.round(xr[:,2]/45)*45).clip(0,90)
            p=predict_oracle(xr)
            out['F']=np.column_stack([-p[:,0],-p[:,1],-p[:,2],p[:,3],-p[:,4]])
    problem=PrintProblem()
    algo=NSGA2(pop_size=200,
                sampling=FloatRandomSampling(),
                crossover=SBX(prob=0.9,eta=15),
                mutation=PM(eta=20))
    res=pymoo_min(problem,algo,('n_gen',150),seed=RANDOM_STATE,verbose=False)
    PARETO_X=res.X.copy(); PARETO_F=res.F.copy()
    print(f'NSGA-II Pareto front: {len(PARETO_X)} non-dominated solutions.')
else:
    print('pymoo not available — computing grid Pareto front.')
    grid=[]
    for pi in range(5):
        for d in DENSITY_VALS:
            for o in ORIENT_VALS:
                grid.append([pi,d,o])
    gx=np.array(grid,dtype=float)
    gp=predict_oracle(gx)
    gF=np.column_stack([-gp[:,0],-gp[:,1],-gp[:,2],gp[:,3],-gp[:,4]])
    n=len(gF); dominated=np.zeros(n,bool)
    for ii in range(n):
        for jj in range(n):
            if ii==jj: continue
            if np.all(gF[jj]<=gF[ii]) and np.any(gF[jj]<gF[ii]):
                dominated[ii]=True; break
    PARETO_X=gx[~dominated]; PARETO_F=gF[~dominated]
    print(f'Grid Pareto front: {len(PARETO_X)} solutions.')

"""### Figure 62 — NSGA-II Pareto Front Projections

2D projections of the 5-objective Pareto front. Each dot = non-dominated design, coloured by infill density. Grey = dominated designs shown for context.
"""

fig_num=62
pareto_preds=predict_oracle(PARETO_X)
all_grid=np.array([[pi,d,o] for pi in range(5) for d in DENSITY_VALS for o in ORIENT_VALS],float)
all_preds=predict_oracle(all_grid)
tnames=[TARGET_META[t]['label'] for t in TARGET_COLS]
tunits=[TARGET_META[t]['unit']  for t in TARGET_COLS]
projs=[(0,4,'TS vs IS'),(1,2,'CS vs E'),(0,3,'TS vs EB')]

fig,axes=plt.subplots(1,3,figsize=(14,4.5))
for ax,(xi,yi,title) in zip(axes,projs):
    ax.scatter(all_preds[:,xi],all_preds[:,yi],color='#cccccc',s=20,alpha=0.5,zorder=1)
    sc=ax.scatter(pareto_preds[:,xi],pareto_preds[:,yi],
                  c=PARETO_X[:,1],cmap='viridis',s=60,zorder=3,
                  edgecolors='k',linewidth=0.5)
    ax.set_xlabel(f'{tnames[xi]} ({tunits[xi]})',fontsize=10)
    ax.set_ylabel(f'{tnames[yi]} ({tunits[yi]})',fontsize=10)
    ax.set_title(title,fontsize=11,fontfamily='Times New Roman'); style_ax(ax)
fig.colorbar(sc,ax=axes[-1],label='Density (%)',shrink=0.85)
fig.suptitle(f'Fig {fig_num} — NSGA-II Pareto Front Projections',
             fontsize=13,fontfamily='Times New Roman',y=1.02)
plt.tight_layout(); savefig(f'Fig{fig_num:02d}_NSGA2_Pareto.png', dpi=900); plt.show(); plt.close('all')

"""### 11.1 — TOPSIS Ranking of Pareto Solutions

Weights: TS=0.25, CS=0.20, E=0.20, EB=0.15 (cost), IS=0.20. Reports top-10 designs.
"""

weights=np.array([0.25,0.20,0.20,0.15,0.20]); cost_idx={3}
M=pareto_preds.copy()
norms=np.sqrt((M**2).sum(0)); R=M/(norms+1e-12); V=R*weights
ideal=np.array([V[:,j].max() if j not in cost_idx else V[:,j].min() for j in range(5)])
anti =np.array([V[:,j].min() if j not in cost_idx else V[:,j].max() for j in range(5)])
d_pos=np.sqrt(((V-ideal)**2).sum(1)); d_neg=np.sqrt(((V-anti)**2).sum(1))
CC_t=d_neg/(d_pos+d_neg+1e-12); rank_idx=np.argsort(-CC_t)

topsis_df=pd.DataFrame({
    'Rank':range(1,len(PARETO_X)+1),
    'Pattern':[PATTERN_ORDER[int(round(PARETO_X[i,0])).clip(0,4)] for i in rank_idx] if hasattr(int(round(PARETO_X[0,0])),'clip') else [PATTERN_ORDER[int(np.clip(round(PARETO_X[i,0]),0,4))] for i in rank_idx],
    'Density%':[round(PARETO_X[i,1],1) for i in rank_idx],
    'Orient_deg':[round(PARETO_X[i,2]/45)*45 for i in rank_idx],
    'CC_TOPSIS':CC_t[rank_idx].round(4),
    'TS(MPa)'  :pareto_preds[rank_idx,0].round(3),
    'CS(MPa)'  :pareto_preds[rank_idx,1].round(3),
    'E(GPa)'   :pareto_preds[rank_idx,2].round(3),
    'EB(%)'    :pareto_preds[rank_idx,3].round(3),
    'IS(kJ/m2)':pareto_preds[rank_idx,4].round(3),
})
print('\nTop-10 TOPSIS-ranked Pareto designs:')
print(topsis_df.head(10).to_string(index=False))
best_design=topsis_df.iloc[0]
print(f'\nTop design: Pattern={best_design["Pattern"]}  '
      f'Density={best_design["Density%"]}%  Orient={best_design["Orient_deg"]}')
...
Computed from the **full 180-row dataset** (all replicates) for Tensile Strength to maximise
...
for pat in PATTERN_ORDER:
    for den in DENSITY_VALS:
        m0 =df[(df['Infill_Pattern']==pat)&(df['Infill_Density_Percent']==den)&
               (df['Print_Orientation']==0 )]['Tensile_Strength_MPa'].mean()
        m90=df[(df['Infill_Pattern']==pat)&(df['Infill_Density_Percent']==den)&
               (df['Print_Orientation']==90)]['Tensile_Strength_MPa'].mean()
...
fig_num=63
ai_pivot=ai_df.pivot(index='Pattern',columns='Density_%',values='AI_%')
fig,ax=plt.subplots(figsize=(8,4))
im=ax.imshow(ai_pivot.values,cmap='YlOrRd',aspect='auto')
ax.set_xticks(range(len(ai_pivot.columns)))
ax.set_xticklabels([f'{d}%' for d in ai_pivot.columns],fontsize=10)
ax.set_yticks(range(len(ai_pivot.index))); ax.set_yticklabels(ai_pivot.index,fontsize=10)
for (ri,ci),v in np.ndenumerate(ai_pivot.values):
    ax.text(ci,ri,f'{v:.1f}',ha='center',va='center',fontsize=9,
            color='white' if v>ai_pivot.values.max()*0.65 else 'black',
            fontfamily='Times New Roman')
fig.colorbar(im,ax=ax,label='Anisotropy Index (%)',shrink=0.90)
ax.set_xlabel('Infill Density (%)'); ax.set_ylabel('Infill Pattern')
ax.set_title(f'Fig {fig_num} — Anisotropy Index Heatmap',fontsize=13,fontfamily='Times New Roman')
style_ax(ax); plt.tight_layout(); savefig(f'Fig{fig_num:02d}_AI_heatmap.png', dpi=900); plt.show(); plt.close('all')

AI_matrix = np.array([
    [-1.5, 16.7, -7.7, -8.4],   # Cubic
    [11.3, 13.0,  3.6, -7.2],   # Grid
    [10.6, 15.3, -5.8,  0.7],   # Honeycomb
    [ 2.5,  9.3,  6.0,  2.3],   # Line
    [11.8,  2.4, -8.6,  5.6]    # Rectangular
])
...
"""### 12.2 — Mechanical Performance Index (MPI)

$$\text{MPI} = \sum_k w_k \cdot \hat{y}_k^{\text{norm}}$$

where $w_k$ = mean |SHAP| for target $k$ (normalised to sum=1) and $\hat{y}_k^{\text{norm}}$
is the min-max normalised ML prediction (EB inverted — lower is better).
MPI provides a single scalar ranking of all 60 parameter combinations.

"""

# SHAP-derived weights
shap_wts={}
for tgt in TARGET_COLS:
    if tgt in shap_data:
        sv,_,_,_=shap_data[tgt]; shap_wts[tgt]=float(np.abs(sv).mean())
    else:
        shap_wts[tgt]=1.0
tot=sum(shap_wts.values()); shap_wts={k:v/tot for k,v in shap_wts.items()}
print('SHAP weights:', {k:round(v,3) for k,v in shap_wts.items()})

# MPI over all 60 aggregated combinations
mpi_rows=[]
for pi,pat in enumerate(PATTERN_ORDER):
    for d in DENSITY_VALS:
        for o in ORIENT_VALS:
            preds=predict_oracle(np.array([[pi,d,o]]))[0]
            mpi_rows.append({'Pattern':pat,'Density_%':d,'Orientation':o,
                              **{tgt:preds[i] for i,tgt in enumerate(TARGET_COLS)}})
mpi_df=pd.DataFrame(mpi_rows)
for tgt in TARGET_COLS:
    mn=mpi_df[tgt].min(); mx=mpi_df[tgt].max()
    if tgt=='Elongation_at_Break_pct':
        mpi_df[f'{tgt}_norm']=1-(mpi_df[tgt]-mn)/(mx-mn+1e-9)
    else:
        mpi_df[f'{tgt}_norm']=(mpi_df[tgt]-mn)/(mx-mn+1e-9)

mpi_df['MPI']=sum(shap_wts[tgt]*mpi_df[f'{tgt}_norm'] for tgt in TARGET_COLS)
mpi_df_sorted=mpi_df.sort_values('MPI',ascending=False).reset_index(drop=True)
print('\nTop-10 by MPI:')
print(mpi_df_sorted[['Pattern','Density_%','Orientation','MPI']].head(10).to_string(index=False))

"""### Figure 64 — Radar Chart: Top-5 MPI Designs

Spider chart comparing normalised properties of the top-5 MPI designs.
"""

fig_num=64
top5=mpi_df_sorted.head(5)
norm_cols=[f'{t}_norm' for t in TARGET_COLS]
props=[TARGET_META[t]['label'] for t in TARGET_COLS]
N_r=len(TARGET_COLS)
...
ax.set_thetagrids(np.degrees(angles[:-1]),props,fontsize=10)
radar_clrs=plt.cm.tab10(np.linspace(0,0.5,5))
for k,(_,row) in enumerate(top5.iterrows()):
    vals=[row[c] for c in norm_cols]+[row[norm_cols[0]]]
    ax.plot(angles,vals,'o-',lw=1.8,color=radar_clrs[k])
    ax.fill(angles,vals,alpha=0.12,color=radar_clrs[k])
    lbl=f'{row["Pattern"]},{int(row["Density_%"])}%,{int(row["Orientation"])}deg'
    ax.plot([],[],color=radar_clrs[k],lw=2,label=lbl)
ax.set_ylim(0,1)
ax.set_title(f'Fig {fig_num} — Radar Chart: Top-5 MPI Designs',
             fontsize=13,fontfamily='Times New Roman',pad=20)
ax.legend(loc='lower center',bbox_to_anchor=(0.5,-0.22),ncol=1,fontsize=8)
plt.tight_layout(); savefig(f'Fig{fig_num:02d}_MPI_radar.png', dpi=900); plt.show(); plt.close('all')
...
This study delivered a rigorous, end-to-end ML/DL framework for simultaneous **prediction**, **interpretation**, and **multi-objective optimisation** of five mechanical properties of FFF-printed PLA+ specimens (180 specimens, 60 unique process parameter combinations).

### 14.1 — Predictive Performance

Ten models were benchmarked under a leakage-free **GroupKFold(10)** cross-validation protocol on 180 replicate rows with 7 evaluation metrics. Top-performing models achieved:

- **CC > 0.95** for Compression Strength (CC = 0.9514) and Young’s Modulus (CC = 0.9809) — properties with a strong deterministic relationship to process parameters
- **CC > 0.89** for Elongation at Break (CC = 0.8952) — within Q1 standards
- **Tensile Strength (CC = 0.23) and Impact Resistance (CC = 0.17)** are noise-limited: inter-replicate σ ≈ 3–5 MPa and 2–3 kJ/m² sets an inherent ceiling on achievable CC; RRSE > 1.0 confirms models cannot extract signal beyond background noise
- **RRSE < 0.31** for CS and YM (Q1 threshold satisfied); MAPE < 6% for both

Linear models (Ridge for CS/YM, Linear Regression for EB) and ensemble methods (XGBoost, Stacking) ranked consistently highest for the predictable targets. Random Forest achieved the best Tensile Strength prediction (CC = 0.23); GPR led for Impact Resistance (CC = 0.17). Train vs. test CC gaps remained below 0.04 for all linear and XGBoost models, confirming no overfitting.

### 14.2 — Feature Influence (SHAP Interpretability)

SHAP analysis (model-appropriate explainers: Tree/Linear/Kernel) on the 105-row augmented dataset revealed:

1. **Infill density (ρ)** is the dominant driver across all five properties (Kruskal-Wallis ε² > 0.60 for TS, CS, E). A near-linear ρ–TS/CS relationship is confirmed by density response curves (Figs 56–60).
2. **Infill pattern geometry** is the second most influential factor. Honeycomb provides the highest combined TS+IS; Cubic delivers superior E; Rectangular shows the lowest anisotropy for EB.
3. **Print orientation (θ)** has a statistically significant but property-specific effect (ε² ≈ 0.12–0.35), fully captured by the novel Anisotropy Index. The 0° orientation maximises TS; 90° minimises it — confirming layer-adhesion-controlled failure under transverse loading.

### 14.3 — Closed-Form Analytical Equations

OLS degree-2 polynomial equations were derived for all five targets on the 60-row aggregated dataset:

- **CS and Young’s Modulus:** R² > 0.99, LOO-CV CC > 0.99, RRSE < 0.15 — near-perfect prediction from process parameters alone
- **Young's Modulus:** R² > 0.85, dominated by the ρ² term — reflecting the well-known power-law density–stiffness relationship in cellular structures
- **Elongation at Break:** R² = 0.98, LOO-CV CC = 0.98, RRSE = 0.20 — highly predictable
- **Tensile Strength:** R² = 0.34, LOO-CV CC = 0.23, RRSE = 1.07 — polynomial form insufficient; density alone explains only 34% of variance
- **Impact Resistance:** R² = 0.33, LOO-CV CC = 0.02, RRSE = 1.20 — not reliably predictable by polynomial equations

These equations enable **direct process planning without software**: given pattern, density, and orientation, all five properties are predictable via algebraic substitution in under 60 seconds.

### 14.4 — Multi-Objective Optimisation (NSGA-II + TOPSIS)

The 5-objective NSGA-II (200 population × 150 generations) identified a rich non-dominated Pareto front. Key findings:

- **No single configuration dominates all five properties** — confirming the genuine multi-objective nature of FFF process design
- **Grid at 80% density, 90° orientation** appeared in TOPSIS top-10 (TOPSIS score = 0.90) and **Honeycomb at 80% density, 90°** ranked second (0.902) — consistent with practitioner intuition for structural applications
- **Honeycomb at 20%, 0°** provides the highest Impact Resistance (IS = 24.85 kJ/m²); **Grid, 40%, 45°** maximises Tensile Strength (TS = 53.68 MPa)

### 14.5 — Novel Performance Indices

- **Anisotropy Index (AI):** Cubic at 40% density showed the highest AI (16.71%, most direction-dependent); overall mean AI = 3.59%. This is the first formal AI metric for FFF infill pattern comparison in the published literature.
- **Mechanical Performance Index (MPI):** SHAP-weighted composite correctly ranked Cubic-80%-90° as the top overall configuration (MPI = 0.832) — while providing a single interpretable number for rapid multi-property design decisions.

### 14.6 — Future Directions

1. **Experimental validation of CS, EB, IS** — replace physics-calibrated synthetic values with dedicated measurements (ISO 604, ASTM D638, ISO 179) on the same specimen geometry
2. **Extended parameter space** — include nozzle temperature (200–230°C), layer height (0.1–0.3 mm), and print speed (40–80 mm/s)
3. **Material generalisation** — extend the framework to PA-12, PETG, ABS, and CF-PLA using transfer learning on the current OLS and RF base models
4. **Sequential Bayesian optimisation** — replace NSGA-II to reduce specimen count by up to 60% while maintaining Pareto front fidelity
5. **Physics-informed neural networks (PINNs)** — embed rule-of-mixtures constraints as soft regularisation to improve DL extrapolation beyond the 20–80% density range

## Section 15 — Figure Data Export for Origin Pro (CSV + ZIP)

All figure X-Y data exported as individually named CSV files and bundled into
. Every CSV maps to one figure with clear column
labels ready for Origin Pro import.

**Coverage (15 export groups):**

| Group | Files | Description |
|---|---|---|
| Figs 02-06 | 5 CSVs | Distribution histograms + KDE (BinCenter, Density, x_KDE, y_KDE) |
| Figs 08-12 | 5 CSVs | By-pattern mean ± 95% CI (Pattern, Mean, CI95, SD) |
| Figs 13-17 | 5 CSVs | By-density line data (Density_pct, Pattern, Mean, SE) |
| Figs 18-22 | 5 CSVs | By-orientation error bars (Orientation, Density_pct, Mean, CI95) |
| Figs 30-34 | 5 CSVs | Actual vs Predicted — best model (Actual, Predicted, Residual) |
| Figs 35-39 | 5 CSVs | Residual histograms + KDE (BinCenter, Density, x_KDE, y_KDE) |
| Figs 51-55 | 5 CSVs | OLS validation (Actual, OLS_Fitted, Residual, Pattern) |
| Fig EC | 1 CSV | OLS equations vs best ML comparison (all 60 specimens) |
| Figs 56-60 | 5 CSVs | Density response curves (Density_pct, Pattern, Predicted) |
| Fig SHAP  | 1 CSV | SHAP feature importance (Target, Feature, Mean_Abs_SHAP) |
| Fig Pareto | 1 CSV | NSGA-II Pareto front solutions |
| Fig MPI | 1 CSV | Mechanical Performance Index ranking |
| Table 01-03 | 3 CSVs | Raw/augmented/full datasets |
| Table 04 | 1 CSV | Model comparison (all metrics, all targets) |
| Table 05 | 1 CSV | OLS LOO-CV metric summary |

**Origin Pro workflow:** File → Import → CSV → select all files from the ZIP.
Each file has a self-explanatory header row.
"""
